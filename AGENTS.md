...
The default VM `rustc` may lag behind this repo; install a toolchain that satisfies `rust-version` in `backend/Cargo.toml` (currently **1.85**) before running `cargo` from `backend/`.
...
cargo run
```
Requires `SUPABASE_URL` and `SUPABASE_SERVICE_KEY` (or a `.env` discoverable from the process working directory).
...
### Key gotchas
- The root `.env` is picked up when present; if you run only from `backend/`, set env vars explicitly or place `.env` accordingly.
